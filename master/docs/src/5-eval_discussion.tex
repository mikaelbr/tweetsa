...
The data that was used to train the different hypotheses/models was given by SemSeval'13, who also provided a test set for SMS messages. This task was considered an opportunity to test if the system really was domain semi-independent. The results from the task indicates that the system performed well both when trained with dev set 1,and with dev set 1 and the NTNU set.
